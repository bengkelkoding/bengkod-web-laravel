<x-admin>
    ...
    <div class="container-fluid">
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
                <p class="fw-semibold mb-4"><span class="card-title mr-4">Tabel Informasi Kelas</span><a href="{{route('lecture.class-information.create', $classroom->id)}}" class="btn btn-outline-dark rounded-pill"><i class="ti ti-plus"></i> Tambah Data</a></p>
                ...
                <div class="table-responsive">
                    <table class="table table-striped max-md:min-w-[250vw]">
                        ...
                        <tbody>
                            @forelse($class_informations as $key => $information)
                            <tr>
                                <th scope="row">{{$class_informations->firstItem() + $key}}</th>
                                ...
                                <td>
                                    <div class="flex">
                                        <span class="text-xs font-semibold inline-block py-1 px-2 rounded text-yello-600 bg-yellow-200 uppercase last:mr-0 mr-2">
                                            <a href="{{ route('lecture.class-information.edit', ['idClassroom' => $classroom, 'class_information' => $information]) }}"><i class="ti ti-edit"></i> Edit</a>
                                        </span>
                                        <form method="POST" action="{{ route('lecture.class-information.destroy', ['idClassroom' => $classroom, 'class_information' => $information->id]) }}">
                                            @csrf
                                            @method('DELETE')
                ...
                {{ $class_informations->withQueryString()->links() }}
            </div>
          </div>
        </div>
    </div>
